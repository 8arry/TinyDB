    ...
    try {
        ...
        while (pos < tablesContent.length()) {
            ...
            while (tableEnd < tablesContent.length() && braceCount > 0) {
                if (tablesContent[tableEnd] == '{') braceCount++;
                else if (tablesContent[tableEnd] == '}') braceCount--;
                tableEnd++;
            }
...
std::string PersistenceManager::exportTableToJson(const Table& table, const std::string& tableName) {
...
void PersistenceManager::importTableFromJson(const std::string& json, Database& database, const std::string& tableName) {
    try {
        ...
        throw PersistenceError("Failed to import table '" + tableName + "': " + std::string(e.what()));
    }
}
