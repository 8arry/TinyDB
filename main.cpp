...
private:
    ...
    void executeSelectSimple(sql::SelectStatement* stmt) {
        ...
        auto rows = condition ? db_.selectFrom(stmt->getTableName(), selectColumns, condition)
    ...
    void executeSelectWithJoin(sql::SelectStatement* stmt) {
        ...
        if (stmt->getWhereCondition()) {
            ...
            for (const auto& row : resultRows) {
                ...
                // Build combined schema (simplified: set all as int type, only for column name
                // lookup)
                for (const auto& colName : allColumnNames) {
                    size_t dotPos = colName.find('.');
                    std::string actualColName =
                        (dotPos != std::string::npos) ? colName.substr(dotPos + 1) : colName;
        ...
        std::vector<tinydb::Row> finalRows =
            extractSelectedColumns(resultRows, allColumnNames, selectedColumns);
    ...
    void executeJoinForRow(sql::SelectStatement* stmt, const tinydb::Table& mainTable,
        ...
        for (const auto& joinRow : joinRows) {
            if (evaluateJoinCondition(*joinClause->getOnCondition(), mainTable, mainRow, joinTable,
                                      joinRow)) {
                // Merge rows
    ...
    bool evaluateJoinCondition(const tinydb::Condition& condition, const tinydb::Table& leftTable,
                               const tinydb::Row& leftRow, const tinydb::Table& rightTable,
                               const tinydb::Row& rightRow) {
    ...
    std::vector<tinydb::Row>
    extractSelectedColumns(const std::vector<tinydb::Row>& rows,
...
bool handleSpecialCommand(const std::string& command, SQLExecutor& executor) {
    ...
    try {
        ...
        // HELP command
            ...
            std::cout << "    WHERE (price > 100 AND category = \"Electronics\") OR stock > 150"
                      << std::endl;
            std::cout << "    WHERE price > 50 AND (category = \"Books\" OR category = \"IT\")"
                      << std::endl;
