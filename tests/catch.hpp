...
namespace Catch {
    ...
    public:
        ...
        int runAllTests() {
            ...
            std::cout << std::endl;
            std::cout << "===============================================================================" << std::endl;
            std::cout << "test cases: " << (passed + failed) << " | " 
    ...
    struct TestCaseRegistrar {
        TestCaseRegistrar(const std::string& name, const std::string& tags, std::function<void()> func) {
...
#define TEST_CASE(name, tags) \
    ...
    static Catch::TestCaseRegistrar CATCH_INTERNAL_REGISTRAR_NAME(__LINE__)(name, tags, CATCH_INTERNAL_FUNCTION_NAME(__LINE__)); \
    static void CATCH_INTERNAL_FUNCTION_NAME(__LINE__)()
...
#define REQUIRE(expr) \
    Catch::require_true((expr), #expr, __FILE__, __LINE__)

#define REQUIRE_FALSE(expr) \
    Catch::require_false((expr), #expr, __FILE__, __LINE__)

#define REQUIRE_THROWS_AS(expr, exception) \
    Catch::require_throws_as<exception>([&]{ expr; }, #expr, __FILE__, __LINE__)

#define REQUIRE_NOTHROW(expr) \
    Catch::require_nothrow([&]{ expr; }, #expr, __FILE__, __LINE__)
